...
fn main() -> anyhow::Result<()> {
    ...
    // let
    // obj.prog_mut("fentry_tcp_v4_connect").unwrap().attach()?;
    // obj.prog_mut("fentry_tcp_v6_connect").unwrap().attach()?;
    // obj.prog_mut("fentry_tcp_rcv_state_process").unwrap().attach()?;
    // obj.prog_mut("tcp_destroy_sock").unwrap().attach()?;
    let mut links = vec![];
    ...
    let mut start_ts = 0;
    let poll = PerfBufferBuilder::new(&map)
        .sample_cb(|_, v| {
            handle_event(v, &mut start_ts);
        })
        .build()?;
    println!(
