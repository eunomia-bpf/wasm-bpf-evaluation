...
#ifdef NATIVE_LIBBPF
    struct perf_buffer *buf = perf_buffer__new(bpf_map__fd(skel->maps.events), PERF_BUFFER_PAGES,
                                              handle_event_wrapper, lost_event, NULL, NULL);
#else
    struct bpf_buffer *buf = bpf_buffer__open(skel->maps.events, handle_event, NULL);
#endif
